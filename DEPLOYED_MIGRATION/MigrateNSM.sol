...
contract MigrateNSM is Access {
    ...
    // original gangster?
    mapping(address => bool) internal isOG;
    ...
    function canISell() external view returns (bool canISell_) {
        return _canHolderSell(_msgSender(), 1);
    }
...
//////// DO MATH! //////
    ...
    */
    ...
    {   // subtract => a-b
        return (
            getHoldersMaxSell(holder)
            .sub(getHoldersClaimsAlreadySold(holder))
        );
    }
        //internal
    function getTotalClaimableFLV(address holder)
        internal
        view
        returns (uint256)
    ...
    function getRemainingClaimableFLV(address holder)
        internal
        view
        returns (uint256)
    {   // subtract => a-b
        return getTotalClaimableFLV(holder).sub(claimedFLV[holder]);
    }

    function getCurrentClaimableFLV(address holder)
        internal
        view
        returns (uint256)
    ...
    function toggleDeposits_OICM() external onlyIceCreamMan {
        depositsEnabled ? depositsEnabled = false : depositsEnabled = true;
    }
    ...
    function getMyInfo()
        ...
        returns (
            ...
            uint256 claimedFLV_,
            uint256 remainingClaimableFLV_,
    ...
    {   
        if(isOG[holder]){
            ...
            remainingNSMdeposit_ = snapshotBalance[holder]
                .sub(deposits[holder]);
            completedDeposits_ = completedDeposits[holder];
            maxClaimableFLV = getTotalClaimableFLV(holder);
            currentClaimableFLV = getCurrentClaimableFLV(holder);
            claimedFLV_ = claimedFLV[holder];
            completedClaims_ = completedClaims[holder];
            remainingClaimableFLV_ = getRemainingClaimableFLV(holder);
    ...
    function claim() external onlyOG{
        ...
        uint256 amount = getCurrentClaimableFLV(holder);
    ...
    function contractNSMBalance() external view returns(uint256) {
        return NSM.balanceOf(address(this));
    }
    ...
    */
    ...
    function statusUpdateHolderCompletedClaims(address holder) internal {
        if (getRemainingClaimableFLV(holder) == 0) {
    ...
    function checkHoldersTotalClaimableFLV(address holder) internal view {
        require(
            claimedFLV[holder] <= getTotalClaimableFLV(holder),
    ...
    {
        require(
            amount <= getRemainingClaimableFLV(holder),
    ...
    function checkAndTransferIn(address holder, uint256 amount) internal {       
        require(
            NSM.transferFrom(holder, address(this), amount),
